...
static TEE_Result cmd_generate_keys(uint32_t param_types, TEE_Param params[4])
{
    uint32_t exp_param_types = TEE_PARAM_TYPES(
        TEE_PARAM_TYPE_MEMREF_OUTPUT, // for public key
        TEE_PARAM_TYPE_NONE,
        TEE_PARAM_TYPE_NONE,
        TEE_PARAM_TYPE_NONE
    );
                                          ...
                                          TEE_Param params[4])
{
    ...
    // Verify that the output buffers provided by the normal world are large enough.
...
static TEE_Result cmd_verify_randomness(uint32_t param_types,
                                        TEE_Param params[4])
{
    // param[0] = memref input_bytes
    // param[1] = memref public_key (32 bytes)
    // param[2] = memref randomness (32 bytes)
    // param[3] = memref proof (64 bytes)
    uint32_t exp_param_types = TEE_PARAM_TYPES(
    ...
    // Reconstruct the 96-byte "out_and_proof"
